...
PalmBeachSpider = spiders_pkg.palm_beach_spider.PalmBeachSpider


def test_palm_beach_spider_collects_items():
    sample = Path('tests/fixtures/palm_beach_sample.html').absolute()
    file_url = 'file://' + pathname2url(str(sample))

    html = sample.read_bytes()
    resp = TextResponse(url=file_url, body=html)

    spider = PalmBeachSpider(start_urls=[file_url])
    ...
    for item in items:
        ...
        assert item.get("county") == "palm_beach"
