...
PinellasSpider = spiders_pkg.pinellas_spider.PinellasSpider


def test_pinellas_spider_collects_items():
    sample = Path("tests/fixtures/pinellas_sample.html").absolute()
    file_url = "file://" + pathname2url(str(sample))

    html = sample.read_bytes()
    resp = TextResponse(url=file_url, body=html)

    spider = PinellasSpider(start_urls=[file_url])
    ...
    for item in items:
        ...
        assert item.get("county") == "pinellas"
