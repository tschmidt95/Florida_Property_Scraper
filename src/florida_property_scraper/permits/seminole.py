        ...
        for row in rows[1:]:
            ...
            if len(cells) >= 5:
                try:
                    permit = PermitRecord(
                        county="seminole",
                        parcel_id=cells[0].get_text(strip=True) if len(cells) > 0 else None,
                        address=cells[1].get_text(strip=True) if len(cells) > 1 else None,
                        permit_number=cells[2].get_text(strip=True) if len(cells) > 2 else "",
                        permit_type=cells[3].get_text(strip=True) if len(cells) > 3 else None,
                        status=cells[4].get_text(strip=True) if len(cells) > 4 else None,
                        issue_date=cells[5].get_text(strip=True) if len(cells) > 5 else None,
                        final_date=cells[6].get_text(strip=True) if len(cells) > 6 else None,
                        description=cells[7].get_text(strip=True) if len(cells) > 7 else None,
                        source=source_url,
                        raw=str(row),
                    )
