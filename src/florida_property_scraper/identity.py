...
def compute_property_uid(item: Dict[str, object]) -> Tuple[Optional[str], Optional[str], List[str]]:
    warnings: List[str] = []
    county = (item.get("county") or "").strip() if isinstance(item.get("county"), str) else ""
    if not county:
    ...
    situs = normalize_address(item.get("situs_address")) if isinstance(item.get("situs_address"), str) else ""
    owner = normalize_text(item.get("owner_name")) if isinstance(item.get("owner_name"), str) else ""
    fallback_seed = f"{county}|{situs}|{owner}"
    if not situs and not owner:
        warnings.append("Missing parcel_id and address/owner; fallback identity may be unstable.")
