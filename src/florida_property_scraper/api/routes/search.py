...
if router:
	...
	def search(q: str = "", county: str = "Orange") -> list[SearchResult]:
		...
		q = (q or "").strip().lower()
		rows = [
			{"owner": "John Smith", "address": "123 Main St", "county": county, "score": 92},
			{
			...
			{"owner": "Maria Garcia", "address": "9 Palm Ave", "county": county, "score": 81},
		]
		if not q:
			return rows
		return [
			r
			for r in rows
			if q in r["owner"].lower() or q in r["address"].lower()
		]
