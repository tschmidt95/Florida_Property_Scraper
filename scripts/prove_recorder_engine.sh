...
echo "$ python -m florida_property_scraper triggers run --county seminole --connector recorder_stub --limit 50 --now $NOW_ISO"
python -m florida_property_scraper triggers run --county seminole --connector recorder_stub --limit 50 --now "$NOW_ISO"
...
echo "$ python - <<'PY' (verify recorder triggers + inbox)"
python - <<'PY'
import os
from florida_property_scraper.storage import SQLiteStore
...
try:
    ...
    assert str(TriggerKey.NOTICE_OF_DEFAULT) in keys
    assert str(TriggerKey.LIS_PENDENS) in keys
    assert str(TriggerKey.MORTGAGE_ASSIGNMENT) in keys
