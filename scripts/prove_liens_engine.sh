...
echo "$ python -m florida_property_scraper triggers run --county seminole --connector liens_stub --limit 50 --now $NOW_ISO"
python -m florida_property_scraper triggers run --county seminole --connector liens_stub --limit 50 --now "$NOW_ISO"
...
echo "$ python - <<'PY' (verify liens triggers + inbox)"
python - <<'PY'
import os
from florida_property_scraper.storage import SQLiteStore
